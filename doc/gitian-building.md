...
*Setup instructions for a Gitian build of Soscoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Soscoin
...
- [Building Soscoin Core](#building-soscoin-core)
...
Clone the git repositories for soscoin and Gitian.
...
in the soscoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Soscoin Core
----------------

To build Soscoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the soscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/soscoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/soscoin-project/soscoin
...
For example:
```bash
URL=https://github.com/thrasher-/soscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit soscoin=${COMMIT} --url soscoin=${URL} ../soscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit soscoin=${COMMIT} --url soscoin=${URL} ../soscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit soscoin=${COMMIT} --url soscoin=${URL} ../soscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the soscoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../soscoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/soscoin
SIGPATH=/some/root/path/soscoin-detached-sigs

./bin/gbuild --url soscoin=${BTCPATH},signature=${SIGPATH} ../soscoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.sosc` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/soscoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/soscoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/soscoin-osx-build.assert
```
...
[gitian.sigs.sosc](https://github.com/soscoin-project/gitian.sigs.sosc/) repository, or if that's not possible create a pull
