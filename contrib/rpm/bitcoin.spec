...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 50511
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 50510
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 150511
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 150510
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 50511
	%{_sbindir}/semanage port -d -p tcp 50510
	%{_sbindir}/semanage port -d -p tcp 150511
	%{_sbindir}/semanage port -d -p tcp 150510
	for selinuxvariant in %{selinux_variants}; do
