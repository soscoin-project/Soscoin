...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5552
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5551
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15552
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15551
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 5552
	%{_sbindir}/semanage port -d -p tcp 5551
	%{_sbindir}/semanage port -d -p tcp 15552
	%{_sbindir}/semanage port -d -p tcp 15551
	for selinuxvariant in %{selinux_variants}; do
